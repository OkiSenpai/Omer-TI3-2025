  ...
  fetch("./?json")
    ...
    .then((data) => {
      ...
      data.forEach((item, index) => {
        ...
        let marqueur = L.marker([latitude, longitude]).bindPopup(
          `<h3>${nom}</h3><p>${adresse}</p><p>Places: </p>`
        );
    ...
    <div class="point-name">
        ${nom} <span class="separator">|</span> ${adresse}
    </div>
`;
    ...
    .catch((error) => {
      ...
      document.getElementById(
        "pointList"
      ).innerHTML = `<div class="error">Erreur lors du chargement: ${error.message}</div>`;
    });
}
